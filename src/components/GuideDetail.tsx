        ...
        <main className="flex-1 w-full max-w-md mx-auto px-4 pb-32 pt-4">
          ...
          <section className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-4">
            ...
            <div className="space-y-3 font-medium text-slate-500 text-xs">
              <p>📍 <strong>{t("必须到户口管辖区所属区购买：", "반드시 해당 지역구 봉투를 구매해야 합니다: ", "Must buy bags of your resident district: ")}</strong>{t("一般垃圾袋（通常为白色/淡蓝色）与食物垃圾袋（通常为黄色）有严格的地域管辖，例如买成了“麻浦区”的袋子是不允许在“西大门区”使用的。去便利店（CU/GS25等）向店员说明你所在的区（Gu）购买即可。", "일반 쓰레기 봉투(대체로 흰색/연청색)와 음식물 쓰레기 봉투(대체로 노란색)는 엄격한 지역 제한이 있습니다. 예컨대 '마포구' 봉투는 '서대문구'에서 사용할 수 없습니다. 인근 편의점(CU, GS25 등)에서 거주하는 구(Gu)를 말하고 구매하면 됩니다.", "General waste bags (white/blue) and food waste bags (yellow) have strict district boundaries. For example, Mapo-gu bags cannot be used in Seodaemun-gu. Simply tell the clerk at a convenience store (CU, GS25, etc.) which district (Gu) you live in.")}</p>
        ...
        <main className="flex-1 w-full max-w-md mx-auto px-4 pb-32 pt-4">
          ...
          <section className="relative overflow-hidden rounded-xl h-40 flex items-center justify-center text-center p-4">
            ...
            <div className="relative z-10 text-white">
              <h3 className="font-bold text-xs mb-1">{t("寻找可靠、配有中文翻译的住房中介？", "중국어 소통이 가능한 신뢰할 수 있는 공인중개사를 찾으시나요?", "Looking for reliable real estate agents with Chinese translation?")}</h3>
